...
We can now join the two datasets. We'll be doing a left outer and an inner join, so that we have the option to explore the entire set of NEOs (though this may not be used too much) as well as a the subset of NEOs with a chance to strike Earth.
...
While it's true that the correlation here is weak, it's also true that the number of smaller objects vastly outweighs the larger ones. Would this correlation be strongly if the numbers weren't so skewed? We can see that aside from only one point, nearly all objects at, around or above 500m in diameter are inching closer to the the -2 cutoff for Polermo Scale. Of the entire dataset, only 1 crosses that barrier. 
...
While we were able to scrape together a minor correlation at the end, the only repeatable pattern to be found was the October spike of NEO Approaches. This could be worthy of a further reading, thought I theorize (with only this analysis as proof, for now) it must have something about the angle of the Earth at that time of year.
