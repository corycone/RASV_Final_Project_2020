...
We can now join the two datasets. We'll be doing a left outer and an inner join, so that we have the option to explore the entire set of NEOs as well as a the subsetted one that includes only NEOs with a chance to strike Earth.
